 ...
 *  
 *  Description : 
 *  This file demonstrates identity continuity after failures 
 ...
 *  - Baseline Failure (No Continuity)          t = 1…5 s
 *  - Continuity Enforced Scenarios:
 *      A – Node Crash / Power Loss              t = 10 s (recovery at 12 s)
 *      B – Agent Migration                       t = 20 s (duration 2 s)
 *      C – Process Restart                        t = 30 s
 *      D – Full Teardown + Redeploy               t = 35 s (redeploy at 37 s)
 *  
 *  
 *  
 *  
...
public:
    ...
    void GenerateVisualizationReport() {
        ...
        report << "NETANIM VISUALIZATION GUIDE – SCENARIOS\n";
        report << "========================================\n\n";
        report << "BASELINE FAILURE (t = 1-5s):\n";
        report << "  - Both edge nodes turn MAGENTA → duplicate agents (split-brain)\n";
        report << "  - Control messages double, packet loss may increase\n\n";
        report << "CONTINUITY SCENARIOS (t = 5-50s):\n";
        report << "  - Crash (10s): red → dark gray\n";
        report << "  - Recovery (12s): bright green, verification turns gNB bright green\n";
        report << "  - Migration (20s): gold → secondary turns bright green\n";
        report << "  - Process Restart (30s): red → bright green (same node)\n";
        report << "  - Teardown (35s): dark gray → redeploy (37s) bright green\n\n";
...
// ============================================================================
// SIMULATION MANAGER – orchestrates failures, 
// ============================================================================
...
public:
    ...
    // BASELINE FAILURE (No Continuity Enforcement) – t = 1…5 s
    ...
    // Continuity Enforced Scenarios (no state preservation)
    void ScheduleFailureEvents() {
        // Scenario A – Node Crash / Power Loss 
        ...
        // Scenario D – Full Teardown + Redeploy (t = 35 s, redeploy at 37 s)
        Simulator::Schedule(Seconds(35.0), &SimulationManager::TriggerFullTeardown, this);
    }

    void TriggerNodeCrash() {
        std::cout << "\n[EVENT] Node Crash / Power Loss  (Scenario A)\n";
        if (m_visualizer) {
    ...
    void RecoverFromCrash() {
        std::cout << "[EVENT] Recovering from crash (downtime 2s)\n";
        ...
        m_metricsCollector->RecordRealMetrics("Continuity – Crash Recovery",
                                              m_twinAgent->GetTotalContinuityChecks());
    }

    void TriggerAgentMigration() {
        std::cout << "\n[EVENT] Agent Migration  (Scenario B)\n";
        ...
        m_metricsCollector->RecordRealMetrics("Continuity – Migration",
                                              m_twinAgent->GetTotalContinuityChecks());
    }

    void TriggerProcessRestart() {
        std::cout << "\n[EVENT] Process Restart (Scenario C)\n";
        if (!m_enterpriseAgent || !m_twinAgent) {
        ...
        m_metricsCollector->RecordRealMetrics("Continuity – Process Restart",
                                              m_twinAgent->GetTotalContinuityChecks());
    }

    void TriggerFullTeardown() {
        std::cout << "\n[EVENT] Full Teardown  (Scenario D)\n";
        if (!m_enterpriseAgent || !m_twinAgent) {
    ...
    void RedeploySystem() {
        std::cout << "[EVENT] Redeploying system (downtime 2s)\n";
        ...
        m_metricsCollector->RecordRealMetrics("Continuity – Full Redeploy",
                                              m_twinAgent->GetTotalContinuityChecks());
    }
    ...
    void PrintFinalReport() {
        ...
        std::cout << "║                                                                        ║\n";
        std::cout << "║ Note: No execution state was ever preserved (as requested).           ║\n";
        std::cout << "╚══════════════════════════════════════════════════════════════════════╝\n";
        ...
        report << "All scenarios executed without preserving execution state.\n";
        report.close();
    }
...
int main(int argc, char *argv[]) {
    ...
    for (uint32_t u = 0; u < ueNodes.GetN(); ++u) {
        ...
        ulClient.SetAttribute("Interval", TimeValue(Seconds(0.02)));
    ...
    // Schedule Baseline Failure early (t = 1…5 s)
    ...
    // Schedule Continuity Scenarios (after baseline, at t = 5s we reinit agents)
    Simulator::Schedule(Seconds(5.0), [simManager]() {
        std::cout << "\n=== STARTING IMMORTAL IDENTITY SYSTEM (With Continuity) ===\n";
        // Reinitialize agents (they were destroyed after baseline)
        simManager->InitializeAgents();
        simManager->SchedulePeriodicDuties();
        simManager->ScheduleFailureEvents();
    });

    // Periodic metric recordings
