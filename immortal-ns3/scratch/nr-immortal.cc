 ...
 *
 *  Results are written to 'Results/' directory with:
...
// ============================================================================
// SHA-256 HELPER — deterministic cryptographic fingerprint
// ============================================================================
...
// ============================================================================
// CUSTOM HEADER — sequence + timestamp
// ============================================================================
...
PacketSinkWithStats::PacketSinkWithStats()
    : m_totalRx(0), m_minDelayMs(1e9), m_maxDelayMs(0), m_lost(0), m_lastSeq(0) {}
...
void PacketSinkWithStats::SetLocal(Address local) { m_local = local; }
...
void PacketSinkWithStats::DoDispose(void) { m_socket = 0; Application::DoDispose(); }
...
void PacketSinkWithStats::StopApplication(void) { if (m_socket) m_socket->Close(); }
...
UdpClientWithHeader::UdpClientWithHeader() : m_sent(0) {}
...
void UdpClientWithHeader::SetRemote(Address remote) { m_remote = remote; }
void UdpClientWithHeader::SetInterval(Time interval) { m_interval = interval; }
void UdpClientWithHeader::SetPacketSize(uint32_t size) { m_packetSize = size; }
void UdpClientWithHeader::SetMaxPackets(uint32_t maxPackets) { m_maxPackets = maxPackets; }

void UdpClientWithHeader::DoDispose(void) { m_socket = 0; Application::DoDispose(); }
...
RealMetricsCollector::RealMetricsCollector()
    : m_baselineCaptured(false), m_totalVerificationEvents(0),
      m_lastVerificationResult(false), m_lastSampleTime(0),
      m_lastDlRx(0), m_lastUlRx(0), m_lastLost(0)
{
                                              ...
                                              double downtime, uint32_t verifEvents)
{
    ...
    {
        throughputDegradation = ((m_baselineMetrics.totalThroughputMbps - avgThroughput)
                                 / m_baselineMetrics.totalThroughputMbps) * 100;
...
private:
    ...
    static constexpr size_t FP_PREVIEW_LEN = 20;
...
SimplifiedVisualizer::~SimplifiedVisualizer() { if (m_anim) delete m_anim; }
...
std::string SimplifiedVisualizer::GetFingerprintTrunc(uint32_t nodeId) const
{
    ...
    if (it != m_nodeFingerprints.end() && it->second.length() > FP_PREVIEW_LEN)
        return it->second.substr(0, FP_PREVIEW_LEN) + "...";
    return "N/A";
}
...
void SimplifiedVisualizer::ShowAgentReappears(uint32_t nodeId, const std::string &nodeType, const std::string &fp)
{
    ...
    std::string desc = "Edge " + nodeType + "\n REAPPEARED\nFingerprint: " + fp.substr(0, FP_PREVIEW_LEN) + "...";
                                                           ...
                                                           const std::string &verifiedFp)
{
    ...
    std::string verifierDesc = "gNB-1\nJMC-Origin\n✓ VERIFIED\nFingerprint: " + verifierFp.substr(0, FP_PREVIEW_LEN) + "...";
    std::string verifiedDesc = "Edge Node\nAegis-RAN\n✓ VERIFIED\nFingerprint: " + verifiedFp.substr(0, FP_PREVIEW_LEN) + "...";
                                          ...
                                          const std::string &primaryFp, const std::string &secondaryFp)
{
    if (primaryId)
    {
        ...
        std::string desc = "Edge Primary\n DUPLICATE\nFingerprint: " + primaryFp.substr(0, FP_PREVIEW_LEN) + "...";
    ...
    if (secondaryId)
    {
        ...
        std::string desc = "Edge Secondary\n DUPLICATE\nFingerprint: " + secondaryFp.substr(0, FP_PREVIEW_LEN) + "...";
...
void SimplifiedVisualizer::GenerateVisualizationReport()
{
    ...
    report << "NETANIM VISUALIZATION GUIDE (SHA-256 Fingerprints)\n"
           << "==================================================\n\n"
           << "All fingerprints are SHA-256 (0x + 64 hex chars).\n"
           << "Previews show the first 18 hex chars for readability.\n\n"
           << "SCENARIO 1 – BASELINE FAILURE (t = 1-11s):\n"
           << "  - Both edge nodes turn MAGENTA -> duplicate agents (split-brain)\n"
           << "  - Packet loss is due to real channel effects (shadowing, fading).\n\n"
...
// ============================================================================
// IDENTITY CLASS — SHA-256 cryptographic fingerprint (UPDATED FORMULA)
// ============================================================================
...
public:
    ImmortalIdentity(const std::string &agentData)
    {
        ...
        // SHA256( AGENT_ID + AGENT_NAME + OWNER + IDENTITY_NAMESPACE + GENESIS_TIME + LINEAGE + MISSION )
        std::string preimage = m_id + "|" + m_name + "|" + m_owner + "|"
                             + m_identityHash + "|" + m_genesisTime + "|"
                             + m_lineageReference + "|" + m_mission;
        m_fingerprint = Sha256Hex(preimage);
    }
    ...
    std::string GetMissionHash() const { return m_missionHash; }
    std::string GetPurposePulseHash() const { return m_purposePulseHash; }
...
// ============================================================================
// TWIN AGENT (JMC-Origin) — verification authority
// ============================================================================
...
public:
    ...
    bool VerifyAgentContinuity(Ptr<ImmortalAgent> agent)
    {
        ...
        bool ok = fpOk && linOk;
        std::cout << "[VERIFY:SHA-256] " << agent->GetName()
                  << ": Fingerprint " << (fpOk ? "✓" : "✗")
                  << ", Lineage " << (linOk ? "✓" : "✗")
                  << " -> " << (ok ? "PASS" : "FAIL") << "\n";
...
public:
    ...
    void RunBaselineFailureScenario()
    {
        ...
        Simulator::Schedule(Seconds(11.0), [this]() { EndScenario(); });
    }
    ...
    void TriggerNetworkPartition()
    {
        ...
        if (m_vis)
        {
            ...
            std::string primFp = m_enterprise ? m_enterprise->GetFingerprint() : entId.GetFingerprint();
            m_vis->ShowSplitBrain(m_primaryEdge->GetId(), m_secondaryEdge->GetId(),
                                  primFp, entId.GetFingerprint());
        }

        Simulator::Schedule(Seconds(2.5), [this]() { m_metrics->RecordMetrics("Network Partition – Mid"); });
        Simulator::Schedule(Seconds(4.5), [this]() { m_metrics->RecordMetrics("Network Partition – End"); });
        Simulator::Schedule(Seconds(5.0), [this]() { EndScenario(); });
    }
